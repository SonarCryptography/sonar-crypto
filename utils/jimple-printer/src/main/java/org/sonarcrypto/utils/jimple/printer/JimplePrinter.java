...
public class JimplePrinter {
  ...
  /**
   * Extracts the source filename from a SootClass.
   *
   * @param cl The SootClass
   * @return The source filename or null if not available
   */
  @Nullable
  private String getSourceFileName(SootClass cl) {
    try {
      var sourcePath = cl.getClassSource().getSourcePath();
      if (sourcePath != null && sourcePath.getFileName() != null) {
        return sourcePath.getFileName().toString();
      }
    } catch (Exception e) {
      // If we can't get the source path, return null
    }
    return null;
  }
  ...
  public void printTo(SootClass cl, PrintWriter out, NormalStmtPrinter printer) {
    ...
    // Record class position mapping
    Position classPosition = cl.getPosition();
    if (lineNumberMapper != null && !(classPosition instanceof NoPositionInformation)) {
      lineNumberMapper.recordClassPosition(
          getJimpleLnNum() - 1, // Previous line was the class declaration
          cl.getType().toString(),
          classPosition);
    }
        ...
        if (method.hasBody()) {
          ...
          if (lineNumberMapper != null && !(methodPosition instanceof NoPositionInformation)) {
            lineNumberMapper.recordMethodPosition(
                getJimpleLnNum() - 1, // Previous line was the method signature
          ...
          // Record method position for abstract/native methods
          Position methodPosition = method.getPosition();
          if (lineNumberMapper != null && !(methodPosition instanceof NoPositionInformation)) {
            lineNumberMapper.recordMethodPosition(
  ...
  private void printStmts(StmtGraph<?> stmtGraph, LabeledStmtPrinter printer) {
    ...
    for (Stmt currentStmt : linearizedStmtGraph) {
      ...
      // Record statement position
      Position stmtPosition = currentStmt.getPositionInfo().getStmtPosition();
      if (lineNumberMapper != null && !(stmtPosition instanceof NoPositionInformation)) {
        lineNumberMapper.recordStmtPosition(
            getJimpleLnNum() - 1, // Previous line was the statement
            currentStmt.toString(),
            stmtPosition);
      }
    }
